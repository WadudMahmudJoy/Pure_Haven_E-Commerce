...
const backupDir = path.join(root, "_phase12b_backup_customer_dashboard");
...
mkdirSync(path.join(root, "app/api/customer-orders"), { recursive: true });
mkdirSync(path.join(root, "app/customer/dashboard"), { recursive: true });
mkdirSync(path.join(root, "app/customer/orders"), { recursive: true });
mkdirSync(path.join(root, "components/customer"), { recursive: true });

/**
 * 1. Customer auth API: keep register/login, add GET current user, add logout.
 */
write(
  "app/api/customer-auth/route.ts",
...
/**
 * 5. Patch user login redirect + localStorage phone.
 */
if (existsSync(path.join(root, "app/user-login/page.tsx"))) {
  let loginPage = read("app/user-login/page.tsx");
  ...
  write("app/user-login/page.tsx", loginPage);
}

/**
 * 6. Navbar account icon goes to customer dashboard.
 */
if (existsSync(path.join(root, "components/layout/Navbar.tsx"))) {
  let navbar = read("components/layout/Navbar.tsx");

  navbar = navbar.replaceAll(`href="/login"`, `href="/customer/dashboard"`);
  navbar = navbar.replaceAll(`href='/login'`, `href='/customer/dashboard'`);

  write("components/layout/Navbar.tsx", navbar);
}

console.log("Phase 12B applied: customer dashboard and order history added.");
console.log("Added: /customer/dashboard");
console.log("Added: /customer/orders");
console.log("Added: /api/customer-orders");
console.log("Updated: /api/customer-auth GET/logout support");
console.log("Updated: account icon to customer dashboard");
